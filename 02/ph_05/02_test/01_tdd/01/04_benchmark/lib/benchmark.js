    ...
    function runTests(tests, view, iterations) {
        for (var label in tests) {
            ...
            (function (name, test) {
                ...
                // すべてのテストが実行を終了し、
                // timesオブジェクトに値がセットされていることが保障される
                setTimeout(function () {
                    var start = new Date().getTime();
                    var l = iterations;

                                        //Function.prototype.lengthを使ってループするかどうかを決める
                    if (!test.length) { //プロパティの値が0ならループ内でテスト関数を実行
                        while (l--) {
                            test();
                        }
                    } else {            //0でなければ、関数が引数として反復回数を受け付けているものとみなす
                        test(l);
                    }
