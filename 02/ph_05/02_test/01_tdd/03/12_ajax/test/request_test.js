(function () {
    var ajax = tddjs.ajax; //ajax名前空間をインポートする
    function forceStatusAndReadyState(xhr, status, rs) { //テスト用の要求ヘルパー
        var success = stubFn();
        var failure = stubFn();
        ajax.get("/url", {
            success: success,
    ...
    TestCase("RequestTest", {
        ...
        "test should throw error without url": function () { //URLが必須とされていることをテストする
    ...
    TestCase("GetRequestTest", {
        setUp: setUp,
        tearDown: tearDown,
        "test should define get method": function () { //tddjs.ajax.getが定義されていることを確かめる
