(function () {
    ...
    TestCase("ReadyStateHandlerTest", {
        ...
        "test should call success handler for local requests": // completeコールバックがローカル要求のために呼び出されていることを確かめる
            function () {
        ...
        "test should call failure handler for status 400": // completeコールバックが失敗
            function () {
                var request = forceStatusAndReadyState(this.xhr, 400, 4);
                assert(request.failure);
            },
        "test should not call failure handler for status 200": //completeコールバックが成功
            function () {
                var request = forceStatusAndReadyState(this.xhr, 200, 4);
